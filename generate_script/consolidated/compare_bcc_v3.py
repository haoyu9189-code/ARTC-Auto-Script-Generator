#!/usr/bin/env python3
"""BCC N=2 latest run vs experiment + NS3 baseline. Y-axis adaptive."""
...
y_top = max(new_max, exp_max) * 1.20
...
if new_ is not None:
    ...
    if cutoff > 5:
        ...
        ax.annotate(f'NEW peak\nσ={s[i]:.2f}\nε={e[i]:.3f}',
                    xy=(e[i], s[i]), xytext=(e[i]+0.05, s[i]+0.1),
                    fontsize=9, color='#d62728',
                    arrowprops=dict(arrowstyle='->', color='#d62728', lw=1))

plt.tight_layout()
out = os.path.join(CONS, 'compare_bcc_v3.png')
plt.savefig(out, dpi=140); plt.close(fig)
print(f'NEW peak: {new_max:.3f}, exp peak: {exp_max:.3f}, y_top: {y_top:.3f}')
print('Saved:', out)
